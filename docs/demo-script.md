...
```
[1]
Most A.I. trading agents give a language model the keys, and hope it behaves. This one does the opposite. It's called Vetoed, it trades options on Alpaca paper, and the A.I. is the least trusted component in it.
...
Everything that reaches the broker goes through Alpaca's own M.C.P. server. That is the single write path in the system - one place where an order can be created, and every order that leaves it carries a deterministic identifier, so a restart cannot open the same spread twice.
...
[9]
Here's a real screen. Of the spreads that reached the shortlist, twenty were structurally valid and seven survived every gate.

Q.Q.Q. produced zero. Implied volatility was below realised, so there was nothing to be paid for, and the agent passed.
...
dollars and stands at JUST OVER A HUNDRED AND TWO THOUSAND. Hover the
...
Now the part I most want you to see. Before any shortlist exists, the agent
measures the whole option chain across four underlyings — and on this cycle it
rejected ABOUT FIFTEEN HUNDRED spreads. Every rejection is counted by the reason that stopped it, naming the
contracts: open interest under five hundred, premium under ten cents, delta
outside nought point one to nought point three five, measured edge below two
dollars. Those four numbers are the strategy. It is not an agent looking for a
reason to trade — it spends almost all of its time declining.
...
And this is Alpaca paper trading throughout - it refuses to start against a live account. A contest week proves nothing about profitability. The research motivates the idea. It does not validate this system.
